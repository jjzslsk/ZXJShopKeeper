 ...
 * 初始化时间控件
 * viewId 触发时间控件的组件id
 * type 模式：date日期；datetime日期时间；time时间；ym年月；
 * funCal 回调，返回选择的时间
 * frmName 当前页面的frm名称，用户显示时间控件时，屏蔽当前页面的滑动,以免滑动冲突
 */
function initDatePicker(viewId,type,funCal,frmName){
	var calendar = new datePicker();
	var cdDate = new Date();
	var yealCur=cdDate.getFullYear();
	var maxDate=Number(yealCur)+100;
	calendar.init({
		'trigger': viewId, /*按钮选择器，用于触发弹出插件*/
		'type': type,/*模式：date日期；datetime日期时间；time时间；ym年月；*/
		'frmName':frmName,
		'minDate':'1900-1-1',/*最小日期*/
		'maxDate':maxDate+'-12-31',/*最大日期*/
		'onSubmit':function(){/*确认时触发事件*/
			funCal(calendar.value);
		},
		'onClose':function(){/*取消时触发事件*/
		}
	});
...
window.datePicker = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) {
            ...
            this.frmName= params.frmName;
            this.bindEvent(this.type);
        },
        frmBouncesType:function(isBan){
        	//是否禁止frm的下拉刷新状态
        	//禁止frm下拉刷新状态，防止滑动时间控件与frm的下拉刷新状态冲突
        	//选择完时间后开放frm下拉刷新状态
        	var frmName=this.frmName;
        	if(frmName!=null && frmName!=undefined && frmName!='') {
				api.setFrameAttr({
					name: frmName,
					bounces: isBan
				});
        ...
        bindEvent: function(type) {
            ...
            function popupDate(e) {
            	_self.frmBouncesType(false);
                _self.gearDate = document.createElement("div");
                _self.gearDate.className = "gearDate";
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    '<div class="date_btn_box">' +
                    '<div class="date_btn lcalendar_cancel">取消</div>' +
                    '<div class="date_btn" >选择时间</div>' +
            ...
            function yCtrlInit() {
                ...
                if (/^\d{4}/.test(_self.trigger.value)) {
                    rs = _self.trigger.value;
                    dateArr.yy = rs - _self.minY;
                    // rs = _self.trigger.value.match(/(^|-)\d{1,4}/g);
                    // dateArr.yy = rs[0] - _self.minY;
                } else {
                    dateArr.yy = dateArr.yy + 1900 - _self.minY;
